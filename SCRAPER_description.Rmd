...
I will read in all of the SN transcripts, those are txt files so in a way
we are not even scraping but just reading in 700 or so text files which is 
not really a big deal. 
However: In general it is neice if you ask permission (I did) and don't 
push the website to its limit. the GRC servers are quite beefy and I will 
probably not even make a dent in them.
...
However in this project we can just use read_lines^[This is the readr variant of readLines from base-R, it is much faster then the original]
...
I have now downloaded everything into memory. which is not readly failt tolerant.
it also takes very long, and since I dont' know how long it takes and if the scraper
stops somewhere, I have added a helperfunction tgat prints  to the 
...
You can find more about the extraction in [extracting features](extracting_features)
...
mistakes and correct them later on. So learn from your missteps and put them
